...
efetch() {
	if [[ ! -e ${DISTDIR}/${1##*/} ]] ; then
		...
		pushd "${DISTDIR}" > /dev/null || return 1
	...
	*)
	...
	case ${CHOST}:${DARWIN_USE_GCC} in
		*darwin*:1)
			...
			local ccvers
			local isgcc=""
			ccvers="$(unset CHOST; /usr/bin/gcc --version 2>/dev/null)"
			case "${ccvers}" in
				*"(GCC) 4.2.1 "*)
		...
		*-solaris*)
			local ccvers
			ccvers="$(unset CHOST; gcc --version 2>/dev/null)"
			case "${ccvers}" in
				*"gcc (GCC) 3.4.3"*)
...
do_emerge_pkgs() {
	...
	for pkg in "$@"; do
		...
		local myuse_copy use
		myuse_copy=" ${myuse[*]} "
		for use in ${USE} ; do
			myuse_copy=" ${myuse/ ${use} /} "
			myuse_copy=" ${myuse/ -${use} /} "
			myuse_copy=" ${myuse/ ${use#-} /} "
			myuse_copy+=" ${use} "
		done
		readarray myuse <<< "${myuse_copy}"
...
bootstrap_stage3() {
	...
	if is-rap ; then
		...
		*) break ;;
		...
		LDFLAGS="-L${ROOT}/usr/$(get_libdir) -Wl,--dynamic-linker=${RAP_DLINKER}"
		export LDFLAGS
		BOOTSTRAP_RAP=yes \
		pre_emerge_pkgs --nodeps "${pkgs[@]}" || return 1
...
EOF
	while true ; do
		...
		local realEPREFIX
		realEPREFIX="$(cd "${EPREFIX}" || exit 1; pwd -P)"
...
if [[ -z ${CHOST} ]]; then
	if [[ x$(type -t uname) == "xfile" ]]; then
		case `uname -s` in
			Linux)
				plt="gnu"
				musl_test=( /lib/ld-musl-*.so.1 )
				[[ -e ${musl_test[0]} ]] && plt="musl"
